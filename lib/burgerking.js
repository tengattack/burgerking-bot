...
function *getSurveyUrl() {
  ...
  return parseSurveyUrl(r[1]);
}

function *postSurveyCode(surveyUrl, code) {
  var formdata = {
    JavaScriptEnabled: 1,
    FIP: 'True',
    NextButton: '开始'
...
return yield yreq.post(BK_URL + surveyUrl, formdata);
}
...
function *main(code) {
  var r = { code: -1 };
  var surveyUrl = yield getSurveyUrl();
  if (surveyUrl) {
    var r = yield postSurveyCode(surveyUrl, code);
    if (r[0].statusCode == 302) {
      r.code = 1;
      r.message = '错误的调查码';
      return r;
      ...
      while (true) {
        ...
        if (r[0].statusCode == 302) {
          ...
          r.code = 2;
          r.message = '错误, HTTP代码: ' + r[0].statusCode;
          console.error(new Date(), r[0].statusCode, r[1]);
          break;
        }
      }
      rlog('\n');
